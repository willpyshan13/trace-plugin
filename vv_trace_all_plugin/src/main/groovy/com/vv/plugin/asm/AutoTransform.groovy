package com.vv.plugin.asm

import com.android.annotations.NonNull
import com.android.annotations.Nullable
import com.android.build.api.transform.*
import com.android.build.gradle.internal.pipeline.TransformManager
import com.vv.plugin.GlobalConfig
...
public class AutoTransform extends Transform {
    ...
    static void printCopyRight() {
        ...
        List<com.vv.plugin.bean.AutoClassFilter> autoClassFilterList = GlobalConfig.getAutoClassFilter()
        autoClassFilterList.each {
            com.vv.plugin.bean.AutoClassFilter filter ->
                println '##########                                                    '
    ...
    public void transform(
        ...
        //开始计算消耗的时间
        com.vv.plugin.util.Logger.info("||=======================================================================================================")
        com.vv.plugin.util.Logger.info("||                                                 开始计时                                               ")
        com.vv.plugin.util.Logger.info("||=======================================================================================================")
        def startTime = System.currentTimeMillis()

        if (com.vv.plugin.util.Logger.isDebug()) {
        ...
        inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                com.vv.plugin.util.Logger.info("||-->开始遍历特定jar ${dest.absolutePath}")
                def modifiedJar = modifyJarFile(jarInput.file, context.getTemporaryDir())
                com.vv.plugin.util.Logger.info("||-->结束遍历特定jar ${dest.absolutePath}")
                if (modifiedJar == null) {
            ...
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                com.vv.plugin.util.Logger.info("||-->开始遍历特定目录  ${dest.absolutePath}")
...
//                            Logger.info(target.getAbsolutePath())
                ...
                com.vv.plugin.util.Logger.info("||-->结束遍历特定目录  ${dest.absolutePath}")
            }
        }
        ...
        com.vv.plugin.util.Logger.info("||=======================================================================================================")
        com.vv.plugin.util.Logger.info("||                                       计时结束:费时${cost}秒                                           ")
        com.vv.plugin.util.Logger.info("||=======================================================================================================")
    }
...
//                Logger.info("Jar:className:" + className)
                if (com.vv.plugin.util.AutoMatchUtil.isShouldModifyClass(className)) {
                    modifiedClassBytes = com.vv.plugin.asm.AutoModify.modifyClasses(className, sourceClassBytes)
                }
            }
    ...
    private static File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            String className = com.vv.plugin.util.AutoTextUtil.path2ClassName(classFile.absolutePath.replace(dir.absolutePath + File.separator, ""))
//            Logger.info("File:className:" + className)
            if (com.vv.plugin.util.AutoMatchUtil.isShouldModifyClass(className)) {
                byte[] sourceClassBytes = IOUtils.toByteArray(new FileInputStream(classFile))
                byte[] modifiedClassBytes = com.vv.plugin.asm.AutoModify.modifyClasses(className, sourceClassBytes)
    ...
    private static void printlnJarAndDir(Collection<TransformInput> inputs) {
        ...
        inputs.each { TransformInput input ->
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                com.vv.plugin.util.Logger.info("||项目class目录：${directoryInput.file.absolutePath}")
            }
            input.jarInputs.each { JarInput jarInput ->
                classPaths.add(jarInput.file.absolutePath)
                com.vv.plugin.util.Logger.info("||项目jar包：${jarInput.file.absolutePath}")
            }
        }
    }
}
